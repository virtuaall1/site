...
(async () => {
  ...
  /* ---------- 9в. Отчёты об ошибках ----------
     Sentry весит больше, чем весь наш сайт, поэтому он обязан
     лежать отдельным куском и грузиться только при ошибке. Если
     он окажется в основном бандле, это заметит счётчик трафика, а
     не мы. */
  {
    const main = FILES.filter(f => /^assets\/index-.*\.js$/.test(f));
    const inBundle = main.some(f => /@sentry|sentry\.io\/api/.test(read(f)));
    const src = fs.readFileSync(path.join(ROOT, 'app', 'lib', 'sentry.js'), 'utf8');

    if (inBundle) fail('Sentry попал в основной бандл — он должен грузиться отдельно и только при ошибке');
    else if (!/addEventListener\('error'/.test(src)) fail('Sentry не ловит ошибки — обработчик не повешен');
    else if (!/import\('@sentry/.test(src)) fail('Sentry подключается не отложенно');
    else ok('отчёты об ошибках: пакет грузится только при ошибке');
  }
